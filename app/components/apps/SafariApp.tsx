...
export default function SafariApp() {
	...
	const renderWebContent = () => {
		if (activeTab.url === "about:blank") {
			return (
				<div className="flex flex-col items-center justify-center h-full bg-gray-100 dark:bg-gray-800 text-black dark:text-white">
					<h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">새 탭</h2>
					<div className="flex flex-wrap justify-center gap-4 max-w-2xl">
						{/* 자주 방문하는 사이트 바로가기 */}
						{[
							{ url: "https://www.google.com", name: "Google", color: "bg-blue-100 dark:bg-blue-900", textColor: "text-blue-600 dark:text-blue-300" },
							{ url: "https://www.naver.com", name: "Naver", color: "bg-green-100 dark:bg-green-900", textColor: "text-green-600 dark:text-green-300" },
							{ url: "https://www.youtube.com", name: "YouTube", color: "bg-red-100 dark:bg-red-900", textColor: "text-red-600 dark:text-red-300" },
						].map((site) => (
							<div
								key={site.url}
								className="w-24 h-24 bg-white dark:bg-gray-700 rounded-lg shadow-md flex flex-col items-center justify-center p-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-600"
								onClick={() => loadUrl(site.url)}
							>
								...
								<span className="text-xs text-center text-gray-900 dark:text-gray-100 truncate w-full">{site.name}</span>
							</div>
						))}
					</div>
				</div>
			...
			<div className="relative w-full h-full">
				...
				{activeTab.error ? (
					<div className="flex flex-col items-center justify-center h-full bg-gray-100 dark:bg-gray-800 p-4">
						<div className="bg-white dark:bg-gray-700 rounded-lg shadow-md p-6 max-w-md">
							<h2 className="text-xl font-semibold text-red-600 dark:text-red-400 mb-4">페이지를 로드할 수 없습니다</h2>
							<p className="text-gray-700 dark:text-gray-300 mb-4">{activeTab.error}</p>
							...
							<div className="flex justify-center space-x-4">
								...
								<button className="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors" onClick={() => goHome()}>
									홈으로
								</button>
							</div>
						</div>
					...
					<div className="flex flex-col items-center justify-center h-full bg-white dark:bg-gray-900 p-8">
						<div className="max-w-2xl w-full bg-gray-100 dark:bg-gray-800 rounded-lg shadow-lg p-6">
							<div className="flex items-center mb-6">
								<div className="w-12 h-12 bg-blue-200 dark:bg-blue-800 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-300 mr-4">
									<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
										<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
									</svg>
								</div>
								<div>
									<h2 className="text-xl font-bold text-gray-900 dark:text-white">{getDomainFromUrl(activeTab.url)}</h2>
									<p className="text-gray-600 dark:text-gray-400 text-sm">{activeTab.url}</p>
								</div>
							</div>

							<div className="bg-white dark:bg-gray-700 rounded-lg p-4 mb-6 border border-gray-300 dark:border-gray-600">
								<h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">보안 정책 알림</h3>
								<p className="text-gray-700 dark:text-gray-300 mb-4">
									이 웹사이트는 현재 iframe에서 로드할 수 없습니다. 대부분의 웹사이트는 보안상의 이유로 iframe 내에서 표시되는 것을 제한하고 있습니다.
								</p>
								<div className="bg-blue-100 dark:bg-blue-900 p-3 rounded text-blue-800 dark:text-blue-200 text-sm">
							...
							<div className="bg-gray-200 dark:bg-gray-700 p-4 rounded-lg">
								<h3 className="text-md font-medium text-gray-900 dark:text-white mb-2">웹사이트 정보</h3>
								<div className="grid grid-cols-2 gap-2 text-sm">
									<div className="text-gray-700 dark:text-gray-300">URL:</div>
									<div className="text-gray-900 dark:text-gray-100 font-mono truncate">{activeTab.url}</div>
									<div className="text-gray-700 dark:text-gray-300">도메인:</div>
									<div className="text-gray-900 dark:text-gray-100">{getDomainFromUrl(activeTab.url)}</div>
								</div>
							</div>
							...
							<div className="mt-6 bg-yellow-100 dark:bg-yellow-900 border border-yellow-300 dark:border-yellow-700 rounded-lg p-4">
								<div className="flex items-center mb-2">
									...
									<h3 className="text-md font-semibold text-yellow-800 dark:text-yellow-300">개발 중인 기능</h3>
								</div>
								<p className="text-yellow-800 dark:text-yellow-300 text-sm">현재 보안 정책 문제로 인해 외부 웹사이트를 직접 표시할 수 없습니다. 개발팀이 이 문제를 해결하기 위해 작업 중입니다.</p>
								<div className="mt-3 text-xs text-yellow-800 dark:text-yellow-300 bg-yellow-200 dark:bg-yellow-800 p-2 rounded">
	...
	return (
		<div className="flex flex-col h-full bg-white dark:bg-gray-900 text-black dark:text-white">
			...
			<div className="flex items-center bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
					...
					{tabs.map((tab) => (
						<div
							...
							className={`flex items-center min-w-[120px] max-w-[200px] h-8 px-3 mr-1 rounded-t-md cursor-pointer ${
								tab.id === activeTabId
									? "bg-white dark:bg-gray-900 border-b-0 text-black dark:text-white"
						...
						>
							...
							<button className="ml-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200" onClick={(e) => closeTab(tab.id, e)}>
				...
				<button className="flex-shrink-0 w-8 h-8 flex items-center justify-center text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full mx-1" onClick={createNewTab}>
			...
			<div className="flex items-center p-2 bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
				<button
					className={`w-8 h-8 flex items-center justify-center rounded-full mr-1 ${
						canGoBack ? "text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700" : "text-gray-400 dark:text-gray-500 cursor-not-allowed"
					}`}
					onClick={goBack}
					disabled={!canGoBack}
				>
				...
				<button
					className={`w-8 h-8 flex items-center justify-center rounded-full mr-1 ${
						canGoForward ? "text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700" : "text-gray-400 dark:text-gray-500 cursor-not-allowed"
					}`}
					onClick={goForward}
				...
				<button className="w-8 h-8 flex items-center justify-center text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full mr-1" onClick={refresh}>
				...
				<button className="w-8 h-8 flex items-center justify-center text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full mr-1" onClick={goHome}>
					<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
				...
				<form onSubmit={handleUrlSubmit} className="flex-1 flex">
					<input
						type="text"
						className="flex-1 px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-black dark:text-white bg-white dark:bg-gray-700"
						value={inputUrl}
			...
			{/* 웹 페이지 표시 영역 */}
			<div className="flex-1 bg-white dark:bg-gray-900 overflow-hidden text-black dark:text-white">{renderWebContent()}</div>
		</div>
	);
}
